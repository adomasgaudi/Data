...
export const CHANGELOG: Release[] = [
  ...
  },
  {
    ...
    children: [
      { version: "b.2.0.47", sp: 0.5, note: "Settings panel tightened up: smaller padding and gaps, and the checkboxes now sit inline with their labels instead of stacked above them — so the whole ⚙ menu is much more compact and needs less scrolling." },
      { version: "b.2.0.46", sp: 2, note: "Decline sit-ups can now be logged by arm position, not just weight: when you type “decline sit ups” in Add a set, an “Arm position” dropdown appears (arms on stomach → straight by sides → hands on head → overhead bent → overhead straight, easiest to hardest). The pick is folded into the exercise name so each position tracks as its own variant — handy since moving the arms up is how you make the movement harder without adding weight." },
      { version: "b.2.0.45", sp: 8, note: "Every time-based graph (exercise progress, the compare overlay, workout sets-over-time and the version-history SP line) now has a “⇄ Realistic / Compacted time” toggle in its legend. Realistic keeps the real calendar spacing (with the empty gaps); compacted squeezes the rest weeks and layoffs so every session is roughly evenly spaced and all your sets fit on screen — the dates still show on the axis and tooltips, the x-axis just isn't linear in real time. It's one app-wide switch: flip it on any graph and they all follow, and it's remembered on your device." },
      { version: "b.2.0.44", sp: 0.5, note: "Workouts “By day” list now labels today's session “Today” instead of its date (e.g. instead of “W Jun 3”), so the most recent session is easy to spot." },
      ...
      { version: "b.2.0.37", sp: 8, note: "New app-wide “active exercises” filter on the Index page: pick a frequency tier (e.g. “S — staples”) and the WHOLE app — every list, graph, leaderboard, heatmap and picker — narrows to just your most-trained lifts. Default is off (all exercises). Saved on your device. Per-exercise force-in/out toggles come next." },
      { version: "b.2.0.36", sp: 5, note: "Combinable & comparable lifts now exist as their own synthetic exercises: “SQ mix” merges Squat + Smith-Machine Squat 1:1, and “DL pattern” puts Deadlift, RDL and stiff/straight-leg DL on one curve by scaling each to a deadlift-equivalent (RDL ≈ 80%). The pure lifts are never changed — these are extra, derived lifts. Groundwork; they become selectable next." },
