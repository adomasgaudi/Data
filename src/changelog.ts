...
export const RELEASES: Release[] = [
  ...
  { version: "b.2.4.12", title: "M/W toggle follows the athlete", sp: 1, note: "The Men / Women toggle in the athlete picker no longer has a “both” state — it's always one or the other. It now auto-switches to match whoever you select (pick a man → M, pick a woman → W), and tapping M or W simply narrows the name chips to that sex so you can pick someone else." },
  { version: "b.2.4.11", title: "Single lift = filtered history", sp: 2, note: "Picking ONE exercise no longer drops you into a different-looking drill-in page — it's just a filter, like picking several. The Workout-history now shows the same compact, expandable session rows as the all-exercises view, scoped to that one lift (every past set of it, in context), with the lift's analytics still in the graph on top. So one lift and many lifts behave identically — same view, just filtered. (The old per-lift calculator / technique-scaling drill-in still lives in the Legacy exercises tab under More.)" },
  ...
  { version: "b.2.4.2", title: "Weight Range bars sectioned by rep", sp: 3, note: "Each bar on the Weight Range graph is now split into one section per rep of that set. The bottom section starts at the weight you lifted (e.g. 80 kg = a 1-rep max of 80), and each section above it ends at what that same weight estimates as a 1RM for one more rep — 80×2, 80×3, … up to the top, which is the full estimated 1RM at your logged rep count. Alternating shading and a thin divider tick mark each rep, so a single bar reads as its rep-by-rep 1RM ladder." },
  ...
  { version: "b.2.3.44", title: "Calendar colour-by groups", sp: 3, note: "When the training-year calendar shows all exercises (no filter), a new “Colour” picker decides how the squares are coloured: One colour (the classic single-hue scale), or by Body part, Muscle group, Function, or Exercise — each group getting its own colour and each day painted by the group you trained most that day. The set count still drives the intensity (a busier day is more solid, a lighter day fewer sets). A compact colour key lists the groups, and a day's tooltip names its dominant group." },
  { version: "b.2.3.43", title: "Dense weight ranges shine", sp: 2, note: "On the Weight Range graph, lots of overlapping translucent bars used to just stack into a muddy dark blob. Now, where three or more weight ranges pile up at the same point, that stretch lights up with a bright, glowing core instead of going darker — so the spots you trained most often shine out. One or two overlapping still read as a normal darker bar." },
  { version: "b.2.3.42", title: "Graph legend is a dropdown", sp: 1, note: "When a chart has many series (more than 6 coloured keys — e.g. the Analysis graph with 10 lifts), the legend no longer crams into a tiny scrolling strip above the chart. It collapses into a floating “Legend (N)” dropdown that opens as an overlay, so the colour key is roomy and the chart keeps its space. Tapping a key still shows/hides that line. Charts with only a few series keep the simple inline legend." },
  ...
  { version: "b.2.3.17", title: "Graph plots real data", sp: 8, note: "The Analysis graph no longer shows a fake “sample data” line when nothing is selected. With no exercise picked it plots a real whole-athlete trend (total Volume over time by default — a 1RM across mixed lifts is meaningless, total training volume isn't); pick one lift for its line, two-plus to overlay them. The only empty message now is the honest “No data yet.” for an athlete with zero logged sets." },
  { version: "b.2.3.16", title: "Athlete picker atop Analysis", sp: 0.5, note: "The athlete chooser (the M/W toggle + scrolling name chips) now sits pinned at the very top of the Analysis view, so you pick who you're looking at before anything else. It's the same one picker relocated into Analysis while you're there and handed back to the Legacy exercises tab when you leave — no duplication, one source of truth." },
  { version: "b.2.3.15", title: "Analysis renders on landing", sp: 0.5, note: "Fix: the Analysis panel was the default landing tab but rendered blank until you tapped Analysis in the bottom nav, because the renderer only ran when you switched to it. Now it paints once at boot, and re-renders whenever you change athlete — so it shows up filled the first time you see it." },
  ...
  { version: "b.2.3.9", title: "Metadata inference + audits", sp: 8, note: "The exercise library now fills in its own anatomy. A keyword-inference table reads each lift's name and assigns its joints and movements (and, through those, its planes) — so “Leg Curl”, “Seated Cable Row”, “Hammer Curl” and dozens more get metadata automatically and the Filters in Workout analysis work on them straight away; anything the rules don't recognise is left blank rather than guessed wrong. A one-pass bulk tool can apply this across the whole library without editing lifts one by one. Two new Data-health checks: a “possible duplicates to review” list (look-alike names — shown, never merged automatically) and a “custom-exercise links to fix” report that flags broken variant/group links (a dissolved lift with a missing or self-referential parent, a combined/comparison group with too few members or a member that doesn't exist). 16 new tests; nothing destructive happens automatically." },
  { version: "b.2.3.8", title: "Enhancements backlog", sp: 1, note: "A prioritised future-ideas wishlist (docs/enhancements-backlog.md), kept separate from open bugs and from the technical-debt report." },
  { version: "b.2.3.7", title: "Heatmap week-row size", sp: 0.1, note: "Tuned the timeline heatmap's week-row sizing." },
  { version: "b.2.3.6", title: "Heatmap week rows", sp: 3, note: "The timeline heatmap groups cells into week rows." },
  { version: "b.2.3.5", title: "Heatmap scroll to end", sp: 1, note: "The heatmap scrolls to the most recent week on open." },
  { version: "b.2.3.4", title: "Multi-category heatmap CSS", sp: 0.5, note: "Styling for the multi-category heatmap." },
  { version: "b.2.3.3", title: "Multi-category heatmap", sp: 5, note: "The timeline heatmap can show multiple categories at once." },
  { version: "b.2.3.2", title: "Heatmap set tiers", sp: 2, note: "Heatmap cells are shaded by set-tier intensity." },
  { version: "b.2.3.1", title: "Analysis tab in nav", sp: 2, note: "The Workout-analysis view gets its own entry in the main navigation." },
  ...
  { version: "b.1.13.0", title: "SVG engine from scratch", sp: 8, note: "New from-scratch SVG chart engine; Compare graph migrated." },
  ...
  { version: "b.1.7.2", title: "Export and import", sp: 3, note: "Export/Import backup + custom exercise codes." },
  ...
  { version: "b.1.5", title: "Sets-over-time and hosting", sp: 8, note: "Workouts sets-over-time graph, smoother charts, and live hosting." },
  ...
  { version: "0.35.5", title: "1RM consistency audit", sp: 3, note: "1RM-consistency audit + regression tests." },
