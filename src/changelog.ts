...
export const CHANGELOG: Release[] = [
  ...
  },
  {
    ...
    children: [
      { version: "b.2.1.37", title: "Leverage variants & scaling", sp: 13, note: "Bodyweight lifts that are progressed by ANGLE/HEIGHT, not weight, now track each setting as its own variant. The height you logged in the note — a squat-rack hole (SQ8), centimetres (10cm) or a level (5 level) — is parsed automatically into a concrete exercise like “Push Ups (SQ8)”, across push-ups, inverted rows, rack-pull deadlifts, good mornings, handstands and more (≈80 old push-up notes alone). Add-a-set offers the matching picker (squat-rack hole / level / cm) so new sets tag themselves. Each variant gets a starting bodyweight-% (lower hole = harder = more of your weight) and a new “heights” panel in the exercise view lists every height side by side with its best set and effort 1RM, with an editable % so you can scale them onto one curve and see them line up. Raw data/notes are never edited — it's all done on load." },
