...
- **Recurrence 2 (b.2.9.42, ROOT) — STILL broken on device** ("still tab button dont work #persistent"). The b.2.9.37 touchstart-reset assumed the menu item gets its OWN fresh touchstart to clear the flag. But on mobile the menu opens UNDER the finger during a long-press, so the SAME continuous press releases onto a menu item — no fresh touchstart, the flag is still `true`, and the capture-phase click listener `stopPropagation`'d that menu-item click before the delegated handler could run. **Real root: the suppress logic was allowed to touch clicks INSIDE the menu at all.** Fix: the capture handler now bails (`return`) the instant the click target is inside `#dashTabMenu` — a menu click is ALWAYS an action and is never swallowed, whatever the flag's state. The flag now only ever affects clicks OUTSIDE the menu (the tab release / page). Fix site: the capture-phase `click` listener in `setupDashHandlers` (tagged `PB-38 recurrence 2`).
- **Watch:** any "swallow the next synthetic click" flag is fragile — mobile may not deliver that click, and a continuous long-press can release onto the menu itself. NEVER let such a flag gate clicks inside the menu it guards; suppress only OUTSIDE clicks. Prefer DIRECT buttons over long-press pop-menus for primary actions.
- **Recurrences:** 2 (bubble menu → tab menu touchstart-reset → the in-menu-swallow root fix).
