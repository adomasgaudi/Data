...
- **Recurrence 5 attempt 3 (2026-06-29, b.2.9.316, UI-65):** switched `.addm-passive-grp` from flex to **GRID `2.6rem minmax(0,1fr)`** (same bulletproof pattern as `.addm-inputs`) + `max-width:100%` on `.addm-passive-slot`/`.addm-passive`/`.addm-passive-grp`. Added `src/addmOverflow.test.ts` (happy-dom) asserting no descendant exceeds the card at vw=522. **Confirmed fixed** — owner deployed v.316 and asked to remove the diagnostic (b.2.9.317, UI-66).
- **Recurrences:** 5 (class: PB-24 → PB-48 → PB-49 → PB-52 → PB-53; resolved at b.2.9.316 grid fix + diagnostic removed b.2.9.317).
