...
- **Recurrence 5 (2026-06-29, b.2.9.312):** owner: the Add button and the note input go off-bounds. The CONTAINER cap (above) holds, but the `min-width:0` belt-and-suspenders was applied only to `.addm-lines`/`.addm-line` — it never reached the column PARENT `.wo-addform--modal` nor the `.addm-field` (note) / `.addm-actions` (Add/Cancel) rows, so those could still floor at min-content and get clipped by the card's `overflow-x:hidden`. **Fix:** cascade `min-width:0` to `.wo-addform--modal`, `.addm-field`, `.addm-actions`, `.wo-af-go` and the `.wo-af-note`/`.wo-af-ex` inputs (+ `max-width:100%` on the inputs), and `flex-wrap:wrap` on `.addm-actions` so the buttons wrap rather than spill. INSUFFICIENT on device.
- **Recurrence 5 attempt 2 (2026-06-29, b.2.9.314, UI-64):** on-screen diag pinned the passive ADD-TAG palette (~565px nowrap scroller) as the floor: `.addm-passive-slot w=565 overCard=105` while `card=475 vw=522` — NOT a card-too-wide bug. Added `min-width:0` to `.addm-passive-slot`, `.addm-passive`, `.addm-sugg`. **Still overflowed on device** (owner screenshot at v.314).
- **Recurrence 5 attempt 3 (2026-06-29, b.2.9.316, UI-65):** switched `.addm-passive-grp` from flex to **GRID `2.6rem minmax(0,1fr)`** (same bulletproof pattern as `.addm-inputs`) + `max-width:100%` on `.addm-passive-slot`/`.addm-passive`/`.addm-passive-grp`. Added `src/addmOverflow.test.ts` (happy-dom) asserting no descendant exceeds the card at vw=522. Diagnostic kept until owner confirms banner is clean.
- **Recurrences:** 5 (class: PB-24 → PB-48 → PB-49 → PB-52 → PB-53; PB-53 min-width:0 cascade was incomplete, then flex min-width alone failed — grid minmax(0,1fr) is attempt 3).
