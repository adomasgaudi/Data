...
- **Watch:** a "remember my view" feature has four independent links — (1) the gesture commits, (2) the commit persists, (3) the persisted value survives reload, (4) the restore key (signature) still matches. Here links 2–4 worked; the bug was link 1 committing too LATE (debounced) to beat a re-mount. Persist user-view state on gesture-END, not only on a debounce, whenever the host may re-mount the component.
