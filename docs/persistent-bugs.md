...
- **Real fix (b.2.9.47):** REMOVED long-press entirely (owner: "i think the long click is ruining everything — just add an extra btn"). Each tab is now `[name button] + [⋯ button]`; tapping ⋯ opens the options menu on a normal click (toggles). Deleted the touchstart timer, the contextmenu handler, `dashTabSuppressClick` / `dashTabPressTimer`, and the capture-phase swallow listener — only a plain close-on-outside-click listener remains. Bubble duplicate/delete were already direct ⧉/✕ buttons (b.2.9.36).
- **Watch:** do NOT use long-press + "swallow the next synthetic click" for primary actions — mobile browsers are inconsistent about the follow-up click (it may not fire, or a continuous press releases onto the menu), so the flag sticks and swallows real taps. Use an explicit, visible button.
- **Recurrences:** 3 (bubble menu → tab touchstart-reset → in-menu bail → full long-press removal).
