...
- **Watch:** a "remember my view" feature has four independent links — (1) the gesture commits, (2) the commit persists, (3) the persisted value survives reload, (4) the restore key (signature) matches — PLUS a fifth: (5) the component must not be RE-MOUNTED out from under the live interaction. Reuse the instance across incidental re-renders; only re-mount when the identity (here: the bubble) actually changes.
