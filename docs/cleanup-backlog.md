...
- ✅ **SNAP-11** [CSS] — `touch-action` scroll-trap class (a touch swipe over an element froze page scroll). `.svgc-plot`/`.svgc-svg` → `pan-y` (b.2.7.28); then swept the horizontal scrollers → `pan-x`: `.hm-year` (the reported training calendar), `.wa-sel-pills--scroll`, the athlete strip (L605), `.sitemap-box`, `.data-table-wrap`. Left `touch-action:none` on genuinely-interactive canvases (`.ex-pad` draw pad, `.pose3d` 3D, `.chart-wrap--big`). Candidates if reported: `.grid-check-wrap`, `.panel` overflow-x.
